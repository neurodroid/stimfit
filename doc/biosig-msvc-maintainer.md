...
Windows patching is pinned to a specific upstream biosig tag (`v3.9.3`).
...
1. Checkout/update `src/biosig` to the pinned tag commit (currently `v3.9.3`).
2. Rebase/update your biosig patch branch against that same tag/commit.
3. Export patch queue into this repository:

```powershell
git -C src/biosig format-patch v3.9.3..feature/msvc2022-libbiosig -o cmake/patches/biosig-msvc
```
...
- `STF_BIOSIG_EXPECTED_TAG`: required biosig tag for patching (default `v3.9.3`)
