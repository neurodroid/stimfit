...
function(stf_import_biosig_target)
  ...
  set(_stf_biosig_link_libraries "")

  if(PkgConfig_FOUND)
    pkg_check_modules(PC_LIBBIOSIG QUIET libbiosig)
    if(NOT PC_LIBBIOSIG_FOUND)
      pkg_check_modules(PC_LIBBIOSIG QUIET biosig)
    endif()

    if(PC_LIBBIOSIG_FOUND)
      if(NOT _stf_biosig_library)
        foreach(_stf_biosig_pkg_lib IN LISTS PC_LIBBIOSIG_LINK_LIBRARIES)
          if(IS_ABSOLUTE "${_stf_biosig_pkg_lib}" AND EXISTS "${_stf_biosig_pkg_lib}")
            set(_stf_biosig_library "${_stf_biosig_pkg_lib}")
            break()
          endif()
        endforeach()
        unset(_stf_biosig_pkg_lib)
      endif()
      ...
      if(NOT _stf_biosig_include_dir)
        foreach(_stf_biosig_inc IN LISTS PC_LIBBIOSIG_INCLUDE_DIRS)
      ...
      if(NOT _stf_biosig_include_dir)
        foreach(_stf_biosig_inc IN LISTS PC_LIBBIOSIG_INCLUDEDIR PC_LIBBIOSIG_INCLUDE_DIRS)
          if(EXISTS "${_stf_biosig_inc}/biosig.h")
            set(_stf_biosig_include_dir "${_stf_biosig_inc}")
            break()
          endif()
      ...
      if(PC_LIBBIOSIG_LINK_LIBRARIES)
        set(_stf_biosig_link_libraries ${PC_LIBBIOSIG_LINK_LIBRARIES})
      elseif(PC_LIBBIOSIG_LIBRARIES)
        set(_stf_biosig_link_libraries ${PC_LIBBIOSIG_LIBRARIES})
      endif()
    endif()
  endif()
  ...
  if(_stf_biosig_library)
    ...
    if(_stf_biosig_link_libraries)
      target_link_libraries(stimfit::biosig INTERFACE ${_stf_biosig_link_libraries})
    endif()
    set(BIOSIG_LIBRARY "${_stf_biosig_library}" PARENT_SCOPE)
    set(BIOSIG_INCLUDE_DIR "${_stf_biosig_include_dir}" PARENT_SCOPE)
  endif()

  unset(_stf_biosig_link_libraries)
endfunction()
...
find_package(HDF5 CONFIG QUIET)
if(TARGET hdf5::hdf5-shared OR TARGET hdf5::hdf5 OR TARGET hdf5::hdf5-static OR
   TARGET hdf5-shared OR TARGET hdf5 OR TARGET hdf5-static OR TARGET HDF5::HDF5)
  ...
  elseif(TARGET hdf5-shared)
    target_link_libraries(stimfit::hdf5 INTERFACE hdf5-shared)
  elseif(TARGET hdf5)
    target_link_libraries(stimfit::hdf5 INTERFACE hdf5)
  elseif(TARGET hdf5-static)
    target_link_libraries(stimfit::hdf5 INTERFACE hdf5-static)
  else()
    target_link_libraries(stimfit::hdf5 INTERFACE HDF5::HDF5)
  endif()
  ...
  elseif(TARGET hdf5_hl-shared)
    target_link_libraries(stimfit::hdf5 INTERFACE hdf5_hl-shared)
  elseif(TARGET hdf5_hl)
    target_link_libraries(stimfit::hdf5 INTERFACE hdf5_hl)
  elseif(TARGET hdf5_hl-static)
    target_link_libraries(stimfit::hdf5 INTERFACE hdf5_hl-static)
  elseif(TARGET HDF5::hdf5_hl)
    target_link_libraries(stimfit::hdf5 INTERFACE HDF5::hdf5_hl)
  endif()
...
if(NOT DEFINED STF_DEBUG_LAPACK_DISCOVERY)
  set(STF_DEBUG_LAPACK_DISCOVERY OFF)
endif()

macro(_stf_debug_lapack msg)
  if(STF_DEBUG_LAPACK_DISCOVERY)
    message(STATUS "[lapack-debug] ${msg}")
  endif()
endmacro()

macro(_stf_collect_lapack_link_items out_var)
  set(_stf_tmp_link_items)
  if(TARGET LAPACK::LAPACK)
    list(APPEND _stf_tmp_link_items LAPACK::LAPACK)
  endif()
  if(DEFINED LAPACK_LIBRARIES AND NOT "${LAPACK_LIBRARIES}" STREQUAL "")
    list(APPEND _stf_tmp_link_items ${LAPACK_LIBRARIES})
  endif()
  list(REMOVE_DUPLICATES _stf_tmp_link_items)
  set(${out_var} ${_stf_tmp_link_items})
  unset(_stf_tmp_link_items)
endmacro()

macro(_stf_collect_blas_link_items out_var)
  set(_stf_tmp_link_items)
  if(TARGET BLAS::BLAS)
    list(APPEND _stf_tmp_link_items BLAS::BLAS)
  endif()
  if(DEFINED BLAS_LIBRARIES AND NOT "${BLAS_LIBRARIES}" STREQUAL "")
    list(APPEND _stf_tmp_link_items ${BLAS_LIBRARIES})
  endif()
  list(REMOVE_DUPLICATES _stf_tmp_link_items)
  set(${out_var} ${_stf_tmp_link_items})
  unset(_stf_tmp_link_items)
...
set(_stf_lapack_provider_candidates)
if(STF_LAPACK_PROVIDER STREQUAL "AUTO")
  if(APPLE)
    list(APPEND _stf_lapack_provider_candidates ACCELERATE OPENBLAS LEGACY)
  else()
    list(APPEND _stf_lapack_provider_candidates OPENBLAS LEGACY)
  endif()
else()
  list(APPEND _stf_lapack_provider_candidates "${STF_LAPACK_PROVIDER}")
endif()

set(_stf_lapack_selected_provider "UNRESOLVED")
set(_stf_lapack_imported_location "")
set(_stf_lapack_link_libraries "")
set(_stf_lapack_use_interface_target ON)
set(_stf_lapack_is_openblas OFF)

foreach(_stf_lapack_provider IN LISTS _stf_lapack_provider_candidates)
  if(_stf_lapack_provider STREQUAL "ACCELERATE")
    if(APPLE)
      find_library(ACCELERATE_FRAMEWORK Accelerate)
      if(ACCELERATE_FRAMEWORK)
        set(_stf_lapack_selected_provider "ACCELERATE")
        set(_stf_lapack_link_libraries "-framework Accelerate")
        set(_stf_lapack_use_interface_target ON)
        break()
      endif()
    endif()
  elseif(_stf_lapack_provider STREQUAL "OPENBLAS")
    find_library(OPENBLAS_LIBRARY NAMES openblas)
    _stf_debug_lapack("OPENBLAS provider: OPENBLAS_LIBRARY='${OPENBLAS_LIBRARY}'")

    set(_stf_had_bla_vendor OFF)
    if(DEFINED BLA_VENDOR)
      set(_stf_saved_bla_vendor "${BLA_VENDOR}")
      set(_stf_had_bla_vendor ON)
    endif()
    set(BLA_VENDOR OpenBLAS)
    _stf_reset_lapack_blas_find_state()
    find_package(LAPACK QUIET)
    _stf_debug_lapack("OPENBLAS/LAPACK probe: LAPACK_FOUND='${LAPACK_FOUND}' LAPACK_LIBRARIES='${LAPACK_LIBRARIES}'")
    if(LAPACK_FOUND)
      _stf_collect_lapack_link_items(_stf_lapack_candidates)
      _stf_debug_lapack("OPENBLAS/LAPACK candidates='${_stf_lapack_candidates}'")
      if(_stf_lapack_candidates)
        set(_stf_lapack_selected_provider "OPENBLAS")
        set(_stf_lapack_link_libraries ${_stf_lapack_candidates})
        set(_stf_lapack_use_interface_target ON)
        set(_stf_lapack_is_openblas ON)
        if(_stf_had_bla_vendor)
          set(BLA_VENDOR "${_stf_saved_bla_vendor}")
        else()
          unset(BLA_VENDOR)
        endif()
        unset(_stf_saved_bla_vendor)
        unset(_stf_had_bla_vendor)
        unset(_stf_lapack_candidates)
        break()
      endif()
      unset(_stf_lapack_candidates)
    endif()
    ...
    _stf_debug_lapack("OPENBLAS/BLAS probe: BLAS_FOUND='${BLAS_FOUND}' BLAS_LIBRARIES='${BLAS_LIBRARIES}'")
    if(BLAS_FOUND)
      _stf_collect_blas_link_items(_stf_blas_candidates)
      _stf_debug_lapack("OPENBLAS/BLAS candidates='${_stf_blas_candidates}'")
      _stf_debug_lapack("OPENBLAS/BLAS candidates are ignored for LAPACK provider selection (BLAS-only is insufficient)")
      unset(_stf_blas_candidates)
    endif()

    find_library(LAPACK_LIBRARY NAMES lapack lapack3 lapack-3)
    if(OPENBLAS_LIBRARY AND LAPACK_LIBRARY)
      set(_stf_lapack_selected_provider "OPENBLAS")
      set(_stf_lapack_link_libraries "${LAPACK_LIBRARY};${OPENBLAS_LIBRARY}")
      set(_stf_lapack_use_interface_target ON)
      set(_stf_lapack_is_openblas ON)
      _stf_debug_lapack("OPENBLAS fallback selected pair LAPACK='${LAPACK_LIBRARY}' BLAS='${OPENBLAS_LIBRARY}'")
      if(_stf_had_bla_vendor)
        set(BLA_VENDOR "${_stf_saved_bla_vendor}")
      else()
        unset(BLA_VENDOR)
    ...
    if(OPENBLAS_LIBRARY)
      _stf_debug_lapack("OPENBLAS fallback rejected because no LAPACK companion library was found")
    endif()
    ...
    unset(_stf_saved_bla_vendor)
    unset(_stf_had_bla_vendor)
  elseif(_stf_lapack_provider STREQUAL "LEGACY")
    _stf_reset_lapack_blas_find_state()
    find_package(LAPACK QUIET)
    _stf_debug_lapack("LEGACY/LAPACK probe: LAPACK_FOUND='${LAPACK_FOUND}' LAPACK_LIBRARIES='${LAPACK_LIBRARIES}'")
    if(LAPACK_FOUND)
      _stf_collect_lapack_link_items(_stf_lapack_candidates)
      _stf_debug_lapack("LEGACY/LAPACK candidates='${_stf_lapack_candidates}'")
      if(_stf_lapack_candidates)
        set(_stf_lapack_selected_provider "LEGACY")
        set(_stf_lapack_link_libraries ${_stf_lapack_candidates})
        set(_stf_lapack_use_interface_target ON)
        unset(_stf_lapack_candidates)
        break()
    ...
    _stf_reset_lapack_blas_find_state()
    find_package(BLAS QUIET)
    _stf_debug_lapack("LEGACY/BLAS probe: BLAS_FOUND='${BLAS_FOUND}' BLAS_LIBRARIES='${BLAS_LIBRARIES}'")
    if(BLAS_FOUND)
      _stf_collect_blas_link_items(_stf_blas_candidates)
      _stf_debug_lapack("LEGACY/BLAS candidates='${_stf_blas_candidates}'")
      _stf_debug_lapack("LEGACY/BLAS candidates are ignored for LAPACK provider selection (BLAS-only is insufficient)")
      unset(_stf_blas_candidates)
    endif()
    ...
    if(LAPACK_LIBRARY)
      set(_stf_lapack_selected_provider "LEGACY")
      set(_stf_lapack_imported_location "${LAPACK_LIBRARY}")
      set(_stf_lapack_use_interface_target OFF)
      break()
    endif()
  endif()
endforeach()

if(_stf_lapack_selected_provider STREQUAL "UNRESOLVED")
  message(FATAL_ERROR
    "Could not resolve LAPACK provider for STF_LAPACK_PROVIDER='${STF_LAPACK_PROVIDER}'. "
    "Requested candidates: ${_stf_lapack_provider_candidates}")
endif()

if(_stf_lapack_use_interface_target)
  add_library(stimfit::lapack INTERFACE IMPORTED)
  target_link_libraries(stimfit::lapack INTERFACE ${_stf_lapack_link_libraries})
else()
  add_library(stimfit::lapack UNKNOWN IMPORTED)
  set_target_properties(stimfit::lapack PROPERTIES IMPORTED_LOCATION "${_stf_lapack_imported_location}")
endif()

target_compile_definitions(stimfit::lapack INTERFACE HAVE_LAPACK)
if(_stf_lapack_is_openblas)
  target_compile_definitions(stimfit::lapack INTERFACE WITH_OPENBLAS)
endif()

unset(_stf_lapack_provider)
unset(_stf_lapack_provider_candidates)
unset(_stf_lapack_selected_provider)
unset(_stf_lapack_imported_location)
unset(_stf_lapack_link_libraries)
unset(_stf_lapack_use_interface_target)
unset(_stf_lapack_is_openblas)
unset(ACCELERATE_FRAMEWORK)
unset(OPENBLAS_LIBRARY)
unset(LAPACK_LIBRARY)
  ...
  if(Python3_Interpreter_FOUND)
    ...
    if(NOT STF_BUILD_MODULE)
      ...
      if(NOT _stf_wxpy_status EQUAL 0)
        ...
        message(FATAL_ERROR "Embedded Python shell requires wxPython to be importable from Python3_EXECUTABLE. ${_stf_wxpy_err}")
      endif()
      ...
      if(NOT _stf_legacy_shell_status EQUAL 0)
        message(WARNING "wx.py.shell is unavailable; the Legacy backend will not be fully functional at runtime.")
      endif()
      ...
      if(NOT _stf_ipython_status EQUAL 0)
        ...
        message(WARNING "IPython is unavailable; Stimfit will fall back to non-IPython backends at runtime. ${_stf_ipython_err}")
      endif()
    endif()
