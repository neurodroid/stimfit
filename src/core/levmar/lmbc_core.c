...
LNSRCH(int m, LM_REAL *x, LM_REAL f, LM_REAL *g, LM_REAL *p, LM_REAL alpha, LM_REAL *xpls,
        ...
        int *mxtake, int *iretcd, int *linit, LM_REAL stepmx, LM_REAL steptl, LM_REAL *sx)
{
    ...
    *linit = 0;
             ...
             */
    {
        ...
        int worksz, freework=0, issolved, linit = 0;
        /* temp work arrays */
        LM_REAL *e,          /* nx1 */
        *hx,         /* \hat{x}_i, nx1 */
        *jacTe,      /* J^T e_i mx1 */
            ...
            while(1){
                ...
                if(jacTeDp<=-rho*pow(Dp_L2, _POW_/CNST(2.0))){
                    ...
                    LNSRCH(m, p, p_eL2, jacTe, Dp, alpha, pDp, &pDp_eL2, func, fstate,
                            &mxtake, &iretcd, &linit, stepmx, steptl, NULL); /* NOTE: LNSRCH() updates hx */
                    /* append number of iterations to file */
#ifdef _DEBUG
                    pLinit = fopen("/home/cs/stimfit/linit", "a");
                    if ( !pLinit ) printf("Error while opening protocol file");
                    fprintf( pLinit, "%d\n", linit );
                    fclose( pLinit );
#endif
