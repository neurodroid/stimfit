...
wxString stf::ATFError(const wxString& fName, int nError) {
    ...
    ATF_BuildErrorText(nError, fName.char_str(),&errorMsg[0], nMaxLen );
...
void stf::exportATFFile(const wxString& fName, const Recording& WData) {
    ...
    for (int n_c=0;n_c<nColumns;++n_c) {
        ...
        if (n_c==0) {
            ...
            columnTitle = titleStr;
            columnUnits = WData[0].GetYUnits();
        }
        if (!ATF_SetColumnTitle(nFileNum, columnTitle.char_str(), &nError)) {
            wxString errorMsg(wxT("Exception while calling ATF_SetColumnTitle():\n"));
            errorMsg+=ATFError(fName,nError);
            throw std::runtime_error(std::string(errorMsg.char_str()));
        }
        if (!ATF_SetColumnUnits(nFileNum, columnUnits.char_str(),&nError)) {
...
void stf::importATFFile(const wxString &fName, Recording &ReturnData) {
    ...
    if (!ATF_OpenFile(fName.char_str(),ATF_READONLY,&nColumns,&nFileNum,&nError)) {
        wxString errorMsg(wxT("Exception while calling ATF_OpenFile():\n"));
        errorMsg+=ATFError(fName,nError);
        throw std::runtime_error(std::string(errorMsg.char_str()));
    }
    ...
    for (int n_c=timeInFirstColumn;n_c<nColumns;++n_c) {
        ...
        if (n_c-timeInFirstColumn==0) {
            ...
            ReturnData[0].SetYUnits(wxString( &unitsVec[0], wxConvLocal ));
        }
        try {
